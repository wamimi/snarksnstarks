...
struct AgeVerificationCircuit<F: Field>{
    ...
    impl<F: Field> ConstraintSynthesizer<F> for AgeVerificationCircuit<F>{
        fn generate_constraints(
            ...
            let legal_age_var = cs.new_witness_variable(|| Ok(self.legal_age))?;

            cs.enforce_constraint(
                lc!() + age_var, // Left-hand side
                lc!() + legal_age_var, // Right-hand side
                lc!() // This is saying that the left side must be greater than or equal to the right side
            )?;
            Ok(())
        }
    }
